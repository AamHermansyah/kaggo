...
/**
 * Vehicle onboarding and company batch management.
 *
 * The design showed "Vehicle make & Model" and "Device ID" as single inputs,
 * but `POST /admin/vehicles` needs `make` and `model` separately, and needs
 * both an `imei` (manufacturer identity) and a `terminalNo` (the JT/T808
 * addressing ID). They are split here so the form can send a valid payload.
 */
...
export const createBatchSchema = z
  ...
  .refine((values) => values.dropOffCloseTime > values.dropOffStartTime, {
    error: "Closing time must be after the starting time",
