  ...
  React.useEffect(() => {
    if (container) {
      if (typeof container === "object" && "current" in container) {
        setMountNode((container.current as HTMLElement | null) ?? null)
...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        "absolute inset-0 z-50 bg-black/40 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
